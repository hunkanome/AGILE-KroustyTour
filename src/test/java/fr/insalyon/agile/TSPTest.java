...
class TSPTest {
	...
	void testTSP1Element() {
		...
		deliveries.add(new Delivery(new Intersection(0L, coordinates, 0), TimeWindow.getTimeWindow(8)));
		Map<TimeWindow, List<Integer>> deliveriesByTimeWindow = new HashMap<>();
		for (Delivery delivery : deliveries) {
			deliveriesByTimeWindow.computeIfAbsent(delivery.getTimeWindow(), k -> new ArrayList<>())
	...
	void testTSPSameTimeWindow() {
		...
		for (Delivery d : deliveries) {
			deliveriesByTimeWindow.computeIfAbsent(d.getTimeWindow(), k -> new ArrayList<>())
